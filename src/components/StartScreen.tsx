...
const ScreenFooter: React.FC<{ onQuitRequest: () => void; isDemoMode: boolean; }> = ({ onQuitRequest, isDemoMode }) => (
    <div className="mt-8 flex items-center justify-between">
        <div className="bg-amber-800 text-pink-200 font-bold py-2 px-4 rounded-lg shadow-inner text-sm">
            {isDemoMode ? '데모 모드' : '개발자: 황소라(수석교사, 2025)'}
        </div>
        <button 
    ...
    if (!studentInfo) {
        return (
            <div className="animate-fade-in flex flex-col" style={{ minHeight: '65vh' }}>
                ...
                <ScreenFooter onQuitRequest={onQuitRequest} isDemoMode={isDemoMode} />
            </div>
        );
    }
